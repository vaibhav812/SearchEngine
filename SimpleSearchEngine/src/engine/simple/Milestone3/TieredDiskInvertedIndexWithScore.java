...
/**
 * A copy of TieredDiskInvertedIndex that also integrates scoring.
 * @author vaibhavjain
 *
 */
public class TieredDiskInvertedIndexWithScore {
	...
	public TieredDiskInvertedIndexWithScore(String path) {
		try {
			mPath = path;
	...
	public ArrayList<DiskPosting> readPostingsFromFile(RandomAccessFile postingsFile, long postingsPosition) {
		...
		try {
			...
			for (int i = 0; i < documentFrequency; i++) {
				...
				byte[] scoreBytes = ByteBuffer.allocate(8).array();
				postingsFile.read(scoreBytes);
				double score = ByteBuffer.wrap(scoreBytes).getDouble();
				termFreq = VariableByteEncoding.decodeNextNumber(postingsFile);
				postingsList.add(new DiskPosting(docId, termFreq, null, score));
	...
	private ArrayList<DiskPosting> readPostingsWithPositionsFromFile(RandomAccessFile postings, long postingsPosition) {
		...
		try {
			...
			for (int i = 0; i < documentFrequency; i++) {
				...
				postingsList.add(new DiskPosting(docId, termFreq, positions, score));
				lastDocId = docId;
			}
			return postingsList;
		}
